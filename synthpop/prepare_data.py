...
def prepare_data(data_dir, hh_sample_file, hh_marginals_file, per_sample_file,
                 per_marginals_file):
    os.chdir(data_dir)
    ...
    hh_sample = pd.merge(
        hh_sample, hhid, how='left', left_on='group_id', right_on='group_id')
    ...
    per_sample = pd.merge(
        per_sample, pid, how='left', left_on='group_id', right_on='group_id')
    per_marginals = pd.read_csv(per_marginals_file, header=0)
    return (
        hh_sample, hh_marginals, per_sample, per_marginals, hh_var_list,
        per_vars, hhld_units, hh_dims, per_dims, hhid_var, pid_var)
