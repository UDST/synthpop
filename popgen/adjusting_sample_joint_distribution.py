...
def hhld_0_joint_dist(hhid_var, hh_sample, hh_var_list):
    ...
    hhld_0_joint_dist = pd.merge(hhld_0_joint_dist, dist_values, how='left', on='group_id', suffixes=('','_y'))
...
def person_0_joint_dist(pid_var, per_sample, per_vars):
    ...
    agg['frequency'] = 'sum'
    agg['group_id'] = 'min'
    dist_values = dist_values.groupby('person_uniqueid', as_index=False).agg(agg)
    person_0_joint_dist = pd.merge(person_0_joint_dist, dist_values, how='left', on='group_id', suffixes=('','_y'))
...
def adjust_person_frequencies(jd, pums_jd, state, county, tract, bg):
    ...
    jd['puma_prob'] = np.divide(jd.frequency.astype('float'), jd.frequency.sum())
    jd['upper_prob_bound'] = np.divide(0.5, jd.frequency.sum())
    pums_jd['pums_prob'] = np.divide(pums_jd.frequency.astype('float'), pums_jd.frequency.sum())
    ...
    jd['puma_adjustment'][jd.pums_prob > jd.upper_prob_bound] = jd['upper_prob_bound'][jd.pums_prob > jd.upper_prob_bound]
    correction = 1 - (jd.puma_adjustment[jd.puma_prob==0].sum())
    jd.puma_prob[jd.puma_prob != 0] = np.multiply(jd.puma_prob[jd.puma_prob != 0], correction)
    ...
    jd = jd.drop(['upper_prob_bound','puma_prob','pums_prob','puma_adjustment'], axis=1)
...
def adjust_hhld_frequencies(jd, pums_jd, state, county, tract, bg):
    ...
    jd['puma_adjustment'][jd.pums_prob <= jd.upper_prob_bound] = jd.pums_prob[jd.pums_prob <= jd.upper_prob_bound]
    jd['puma_adjustment'][jd.pums_prob > jd.upper_prob_bound] = jd.upper_prob_bound[jd.pums_prob > jd.upper_prob_bound]
    correction = 1 - (jd.puma_adjustment[jd.puma_prob==0].sum())
    jd.puma_prob[jd.puma_prob != 0] = np.multiply(jd.puma_prob[jd.puma_prob != 0], correction)
    jd.puma_prob[jd.puma_prob == 0] = jd.puma_adjustment[jd.puma_prob == 0]
...
def prepare_control_marginals(varcorrdict, marginals, state, county, tract, bg):
    marginals = marginals[(marginals.state==state)&(marginals.county==county)&(marginals.tract==tract)&(marginals.bg==bg)]
    control_marginals = {}
...
def update_weights(vals, joint_dist):
    ...
    val = vals[1]
    adjustment = vals[2]
    joint_dist.frequency[joint_dist[col]==val] = np.multiply(joint_dist.frequency[joint_dist[col]==val], adjustment)        
    return joint_dist
    
    
def adjust_weights(varcorrdict, joint_dist, marginals, ipf_tolerance, ipf_iterations, state, county, tract, bg, pumano):
    control_marginals = prepare_control_marginals(varcorrdict, marginals, state, county, tract, bg)
    tol = 1
    iteration = 0
    adjustment_old = []
    target_adjustment = []
    while tol:
        ...
        tol = tolerance(adjustment_all, adjustment_old, iteration, ipf_tolerance)
        adjustment_old = adjustment_all
        adjustment_characteristic = abs(np.array(adjustment_all) - np.array(target_adjustment)).sum() / len(adjustment_all)
